...
class Host:
    ...
    def __str__(self):
        return (magenta('---- HOST MALWARE ----\n') +
                ...
                '{blue(self.location)}  -> ' +
                '{green(self.tmpdname)}\n' +
                'size: {magenta(self.size)}B\n' +
                'avg. cyclomatic complexity: {magenta(self.avg_cc)}\n' +
                'classes: {blue(pformat(self.classes))}\n' +
                'permissions: {magenta(pformat(self.permissions))}\n' +
                'features: {cyan(pformat(self.features))}\n' +
                magenta('----------------------\n'))
...
class Organ:
    ...
    def __str__(self):
        return (magenta('---- EXTRACTED ORGAN ----\n') +
                '{cyan(self.feature)} @ ' +
                '{blue(self.location)} \n' +
                'donor: {self.donor} @ {self.donor_path}\n' +
                'classes: {blue(pformat(self.classes))}\n' +
                'permissions: {magenta(pformat(self.permissions))}\n' +
                'feature dict: {cyan(pformat(self.feature_dict))}\n' +
                magenta('----------------------\n'))
...
def fetch_harvested(features):
    ...
    logging.info(yellow('{len(harvested)}/{len(features)} organs harvested and ready for transplant!'))
    return harvested
