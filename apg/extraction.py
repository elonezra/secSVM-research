...
def mass_organ_harvest(model, feature_depth, donor_depth):
    ...
    for f, _, _ in model.benign_weights[:feature_depth]:
        ...
        logging.debug(blue('Searching for {f} in donors...'))
        donors = search_for_organ(f, model, donor_depth)
        logging.debug(green('Found {f} in:\n{pformat(donors)}'))
...
def harvest_organ_from_donor(feature, donor_path):
    ...
    if os.path.exists(failure_test):
        logging.warning(red('Previously failed') + ' to extract organ for feature {feature} from {donor_path}')
        return None

    if os.path.exists(pickle_location):
        logging.warning(green('Already extracted') + ' organ for feature {feature} from {donor_path}')
    ...
    logging.debug(yellow('Extracting {j_feature} from {donor_path}...'))

    if not os.path.isfile(donor_path):
        logging.warning(red('Donor app not found: {donor_path}'))
        return None
    ...
    # Weigh the organ
    logging.debug('Evaluating feature {organ.feature}')
...
def extract(apk, j_feature, feature_type):
    ...
    try:
        ...
        logging.debug('Extractor timed out during {apk}, skipping feature')
        raise
        ...
        with open(logfile, 'wt') as f:
            f.write(exception)
        logging.warning(red('Exception during extraction [{logfile}]'))
        raise
    return out
