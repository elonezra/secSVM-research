...
def make_evasive(malware, model, orgs, margin, output_dir):
    ...
    logging.info(green('Host {host.name} loaded!'))

    # Setup output file paths
    records_dir = os.path.join(output_dir, 'records')
    record_name = '{host.name}.record.json'

    features_dir = os.path.join(output_dir, 'adv-features')
    features_name = '{host.name}.adv.json'
    ...
    if predicted_class == 0:
        msg = 'Initial target {host.name} is not predicted as malware! (weird)'
        raise Exception(msg)

    logging.info(blue('Calculating target score and target perturbation...'))
    target_perturbation = np.abs(initial_score - -margin)
    logging.info('{initial_score} - {target_perturbation} = -{margin}')
    ...
    logging.info(yellow('New score: {new_score} (< -{margin})'))
...
def problem_space_transplant(record, model, output_dir):
    ...
    logging.info(green('Host {host.name} ready for operation!'))
    ...
    logging.info('Writing to perm_file: {perm_file}...')
    ...
    logging.debug('Final organs to inplant: {apks}')
    ...
    if 'error' in post_op_host:
        msg = "Error occurred during injection {post_op_host}"
    ...
    if out != 0:
        msg = 'Generated program not predicted as malware'
        raise RetryableFailure(msg)
    ...
    report_path = os.path.join(output_dir, 'success', 'report-{host.name}.json')
    ...
    failure_path = os.path.join(output_dir, 'failure', '{host.name}.txt')
