...
def run_java_component(jar, args, timeout=None):
    ...
    logging.info(blue('Running command') + ': {" ".join(cmd)}')
    ...
    try:
        ...
        logging.warning('Java component {jar} timed out.')
        ...
        logging.warning(
            'SUBPROCESS Extraction EXCEPTION: {exception}')
    return ''
...
def configure_logging(run_tag, debug=True):
    fmt = '[ {run_tag} | %(asctime)s | %(name)s | %(processName)s | %(levelname)s ] %(message)s'
    datefmt = '%Y-%m-%d | %H:%M:%S'
...
def log_failure(malware, msg, output_dir):
    ...
    logging.error('Writing log to {output}')
...
def dump_data(protocol, data, output_dir, filename, overwrite=True):
    ...
    logging.info('Dumping data to {fname}...')
...
def stamp_start_time(output_dir):
    start_time = timer()
    logging.info('Logging start time @ {start_time}')
